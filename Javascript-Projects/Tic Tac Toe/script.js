...
const checkDiagonalsRevers = (currentPlayer) => {
  let count = 0;
...
const checkWin = (currentPlayer) => {
  ...
  if (checkDiagonalsRevers(currentPlayer)) return true;
};
...
const createCell = (index) => {
  const cellElements = `<div class ="cell" role="button" tabindex=${
    index + 1
  }><span class="value"></span></div> `;
  const cellElement = document
    .createRange()
    .createContextualFragment(cellElements);
